...
const AlliedUnitEntryButton = props => {
	...
	let greyedOut = false
	let i
	for (i = 0; i < props.alliedGreyedOutUnits.length; i++) {
		if (parseInt(props.alliedGreyedOutUnits[i].id) === parseInt(props.unit.id)) {
			greyedOut = true
		}
	}
		...
		display =
			<div className={style['unit-entry-button-row']}>
				...
				<div className={style['unit-entry-button-label-div']}>
					<span
						onClick={() => props.addAlliedUnitToList(props.unit, props.alliedArmy)}
